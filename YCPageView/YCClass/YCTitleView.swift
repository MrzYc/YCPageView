...
protocol YCTitleViewDelegate : class {
    func titleView(_ titleView: YCTitleView, didSelected currentIndex : Int)
}
...
class YCTitleView: UIView {
    ...
    fileprivate var style : YCPageStyle
    ...
    // MARK: 构造函数
    init(frame : CGRect, titles : [String], style : YCPageStyle) {
...
extension YCTitleView : YCContentViewDelegate {
    
    func contentView(_ contentView: YCContentView, endScroll inIndex: Int) {
        // 1.取出两个Label
        let oldLabel = titleLabels[currentIndex]
    ...
    func contentView(_ contentView: YCContentView, targetIndex: Int, progress: CGFloat) {
        let oldLabel = titleLabels[currentIndex]
